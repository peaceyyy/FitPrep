...
export default function CheckoutScreen({ plan, user, onBack, onConfirm }) {
  ...
  const visibleScheduleDays = showFullSchedule
    ? scheduleDays
    : scheduleDays.slice(0, 2);
  const remainingScheduleDays = Math.max(
    scheduleDays.length - visibleScheduleDays.length,
    0,
  );
  ...
  const handleConfirm = async () => {
    ...
    if (!deliveryTime) {
      Alert.alert(
        ...
        "Choose either 6:00 AM or 10:00 AM before placing your preorder.",
      );
      return;
    }
    ...
    <ScrollView style={styles.root} contentContainerStyle={styles.content}>
      ...
      <View style={styles.summaryCard}>
        <View style={styles.summaryHeader}>
          <View style={styles.planBadge}>
            <AppText style={styles.planBadgeText}>Plan Selected</AppText>
          </View>
          <AppText style={styles.summaryMeals}>
            {selectedPlanMeals?.length || 21} Total Meals
          </AppText>
        </View>
        ...
        <View style={styles.scheduleContainer}>
          <View style={styles.scheduleHeader}>
            <View>
              ...
              <AppText style={styles.scheduleSubtitle}>
                {showFullSchedule
                  ? "Full week shown"
                  : "Previewing the first two delivery days"}
              </AppText>
            </View>
            <Pressable
                ...
                <View style={styles.mealsGrid}>
                  <View style={styles.mealRow}>
                    ...
                    <View style={styles.mealTextContainer}>
                      <AppText style={styles.mealTypeLabel}>
                        Breakfast:{" "}
                      </AppText>
                      <AppText style={styles.mealNameText}>
                        {dayMeals.Breakfast?.meal_name ||
                  ...
                  <View style={styles.mealRow}>
                    ...
                    <View style={styles.mealTextContainer}>
                      ...
                      <AppText style={styles.mealNameText}>
                        {dayMeals.Lunch?.meal_name || "No lunch scheduled"}
                      </AppText>
                    </View>
                  </View>
                  <View style={styles.mealRow}>
                    ...
                    <View style={styles.mealTextContainer}>
                      ...
                      <AppText style={styles.mealNameText}>
                        {dayMeals.Dinner?.meal_name || "No dinner scheduled"}
                      </AppText>
                    </View>
                  </View>
                </View>
          ...
          <View style={styles.planNote}>
            ...
            <AppText style={styles.planNoteText}>
              Nutritional Focus:{" "}
              {plan?.category === "Cutting"
                ? "Low Carb / High Protein"
                : plan?.category === "Bulking"
        ...
        {timeOptions.map((option) => (
          ...
          >
            ...
            <AppText
              style={[
                styles.timeLabel,
                selectedTime === option.label && styles.timeLabelActive,
              ]}
            >
              {option.label}
            </AppText>
      ...
      <View style={styles.paymentCard}>
        <View style={styles.paymentHeader}>
          ...
          <AppText style={styles.paymentCardTitle}>
            {MOCK_PAYMENT_METHOD} Demo Payment
          </AppText>
        </View>
        ...
        <View style={styles.onlineCardContent}>
          <View style={styles.qrPlaceholder}>
            ...
            <AppText style={styles.qrSubText}>
              Shown for the MVP walkthrough only
            </AppText>
          </View>
          ...
          >
            <Ionicons
              name={paymentMarked ? "checkmark-circle" : "wallet-outline"}
              size={18}
              color="#fff"
              style={{ marginRight: 8 }}
            />
      ...
      <View style={styles.totalCard}>
        <View style={styles.totalRow}>
          ...
          <AppText style={styles.totalValue}>
            ₱
            {Number(plan?.weekly_price || 0).toLocaleString("en-US", {
              minimumFractionDigits: 2,
            })}
          </AppText>
        </View>
        <View style={styles.totalRow}>
          <AppText style={styles.totalLabel}>Delivery Fee</AppText>
          <AppText style={[styles.totalValue, styles.freeLabel]}>FREE</AppText>
        </View>
        <View style={styles.divider} />
        <View style={styles.totalRow}>
          <AppText style={styles.totalTitle}>Total</AppText>
      ...
      {!preorderEligibility.canPreorder && (
        <View style={styles.checkoutNotice}>
          <AppText style={styles.checkoutNoticeText}>
            {preorderEligibility.reason}
          </AppText>
        </View>
      )}
      ...
      >
        {loading ? (
          ...
          <AppText
            style={[
              styles.confirmLabel,
              !canConfirm && styles.confirmLabelDisabled,
            ]}
          >
            {canConfirm ? "Place Preorder →" : "Preorder Locked"}
          </AppText>
        )}
      </Pressable>
    </ScrollView>
